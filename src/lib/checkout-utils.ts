...
export function formatOlvaDistrictName(name: string) {
  const normalized = name.trim();
  return normalized.replace(/^Cercado de /i, "");
}
...
export function toIzipayContact(
  account: UserAccount,
  ...
  return {
    firstName: account.name || address?.receiverName || "Cliente",
    lastName: `${account.paternalSurname} ${account.maternalSurname ?? ""}`.trim() || address?.receiverName || "Sweet Silvia",
    email: account.email,
    phoneNumber: address?.receiverPhone || account.phone || "",
    street: address?.line || fallbackStreet,
    postalCode,
    city: districtName,
